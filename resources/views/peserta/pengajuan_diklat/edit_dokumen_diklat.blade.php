        ...
        <div class="page-title-box">
            <div class="page-title-right">
                <ol class="breadcrumb m-0">
                    ...
                    <li class="breadcrumb-item active">Edit Dokumen Pengajuan Diklat </li>
                </ol>
            </div>
            <h4 class="page-title">Edit Dokumen Pengajuan Diklat</h4>
        </div>
    </div>
</div> 
<div class="row">
                      ...
                      @foreach ($dokumen as $i => $item)  
                            ...
                            <tr>
                                ...
                                <td data-title="dokumen" style="width: 75%;">
                                    <a class="action-icon"><button type="button" class="btn btn-primary btn-sm" data-bs-toggle="modal" data-bs-target="#centermodal" data-lihat="{{$item->id}}" id="upl_dokc_pengajuan" style="display: inline-block; margin-top:8px;">Edit</button></a> 
                                </td>
                            </tr>
                      @endforeach                     
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header">
                <h4 class="modal-title" id="myCenterModalLabel">Edit Dokumen Pengajuan</h4>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-hidden="true"></button>
            </div>
            <form action="#" method="POST" id="form_dokcc_pengajuan" enctype="multipart/form-data">
                @csrf
            <div class="modal-body">
                    <input type="hidden" id="dokcc_id_pengajuan">
                    ...
                    <div class="mb-3 text-left">
                        <button class="btn btn-primary" type="submit" id="simpan_dokcc_pengajuan">Simpan</button>
...
$(document).on('click', '#upl_dokc_pengajuan', function(e) {
        e.preventDefault();
        var id = $(this).attr('data-lihat');
        console.log(id);
        $.ajax({
            url: "{{ url('/get_ubah_dokumen_pengajuan') }}/" + id,
            method: "GET",
            data: {
                id: id,
                _token: '{{ csrf_token() }}'
            ...
            success:function(response) {
                ...
                $('#dokcc_id_pengajuan').val(response.data.id);
            }
        });
    }); 
    $('#form_dokcc_pengajuan').submit(function(e) {
        e.preventDefault();

        //define variable
        let id        = $('#dokcc_id_pengajuan').val(); 
        console.log(id);
        let dt_form   = new FormData(this);
        $('#simpan_dokcc_pengajuan').text('Proses...');
        $.ajax({
            url: "{{ url('/ubah_doc_pengajuan_peserta') }}/" + id,
            method: "POST",
            cache: false,
            contentType: false,
            processData: false,
