        ...
        <div class="page-title-box">
            <div class="page-title-right">
                <ol class="breadcrumb m-0">
                    <li class="breadcrumb-item"><a href="javascript: void(0);">Diklat Riwayat</a></li>
                    <li class="breadcrumb-item active">Verifikasi Pendaftaran Diklat</li>
                </ol>
            </div>
            <h4 class="page-title">Verifikasi Pendaftaran Diklat</h4>
        </div>
    </div>
</div>  
<div class="row">
    <div class="col-xl-12">
        <div class="card">
            <div class="card-body">
                <h4 class="header-title" style="text-align: center; color:#000">Verifikasi Pendaftaran Diklat</h4><br>
                  ...
                  <tbody>
                    <tr>
                      ...
                      <td class="formulir-border" style="padding-left:0.8em; width:100%">
                        <a class="action-icon"><button type="button" class="btn btn-success lihat_diklat" data-bs-toggle="modal" data-bs-target="#centermodal" data-lihat="{{$datas->id}}" id="valid_dftr" style="display: inline-block; margin-top:8px; width:100%">Verifikasi</button></a> 
                        </td>
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            ...
            <div class="modal-body">
                    <input type="hidden" id="daftar_idd">
                    <div class="mb-3">
                        <label for="sts_verif" class="form-label">Aksi</label>
                        <select class="form-select aksi-val" id="sts_verif">
                    ...
                    <div class="mb-3" id="catatan-valid" style="display: none">
                        <label for="ctt_verif" class="form-label">Catatan</label>
                        <textarea class="form-control" id="ctt_verif" rows="5"></textarea>
                    </div>

                    <div class="mb-3 text-left">
                        <button class="btn btn-primary" type="button" id="valid_kirim">Kirim</button>
...
<script>
    $(document).ready(function() {
     ...
     $(document).on('click', '#valid_dftr', function(e) {
        e.preventDefault();
        var id = $(this).attr('data-lihat');
        $.ajax({
            url: "{{ url('/get_edit_daftar') }}/" + id,
            method: "GET",
            success:function(response) {
                $('#daftar_idd').val(response.data.id);
                $('#centermodal').modal('show');
            }
        })
    }); 
    $('#valid_kirim').click(function(e) {
        e.preventDefault();

        //define variable
        let id        = $('#daftar_idd').val();
        let status    = $('#sts_verif').find(":selected").val();
        let catatan   = $('textarea#ctt_verif').val();
        let token     = $("meta[name='csrf-token']").attr("content");

        $.ajax({
            url: "{{ url('/proses_validasi_daftar') }}/" + id,
            method: "POST",
            cache: false,
            data: {
                "status": status,
            ...
            success:function(response) {
                ...
                // location.reload();
                window.location.href = "{{url('/halaman_riwayat_diklat')}}";
            },
